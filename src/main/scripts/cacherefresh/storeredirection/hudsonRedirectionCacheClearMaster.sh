...
OUTPUT_LOG='/tmp/storeRedirectionRefresh_'$(date '+%d_%h_%Y_%H_%M_%S')
...
echo "Refreshing cache for $QUERY on $HOSTNAME"
 ssh -i keymo -oStrictHostKeyChecking=no -l ${USERNAME} ${HOSTNAME} "cd /var/lib/fk-w3-sherlock/semantic-cache-refresh/SemanticCacheRefreshUnix; sudo sh hudsonRedirectionCacheClearUnix.sh $QUERY " 2>$OUTPUT_LOG &
 HOST_NUMBER=$((HOST_NUMBER+1))
done

rm $FILE_NAME

echo "Hudson redirection store cache refresh script is running. Waiting to finish all requests. Error logs in $OUTPUT_LOG"
wait
